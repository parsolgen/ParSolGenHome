    ...
    for (i = 0; i < height; i++) {
        ...
        if (lang == 'c' || lang == 'C') {
            for (j = 0; j < width; j++) {
                ...
                int value = static_cast<int>(1.0 / data[j + i * width] * 255);
                if (value > 255) {
                	value = 255;
                }
                *row++ = value;
          ...
          pixel_t *pix)
{
    ...
    if (ival < 0) {             /* Colder than colorscale, substitute blue */
        ...
        pix->red = 0;         /* Hotter than colormap, substitute red */
        pix->green = 255;
        pix->blue = 255;
    } else {
...
void read_file(int ii, int jj, int block_n, int block_m, int glob_m, double* data)
{
	std::ifstream ifs{"of_" + std::to_string(ii) + "_" + std::to_string(jj), std::ios::binary};
	int in_n = 0;
	int in_m = 0;

	ifs.read(reinterpret_cast<char*>(&in_n), sizeof(int));
	ifs.read(reinterpret_cast<char*>(&in_m), sizeof(int));

	for (int i = 0; i < in_n; ++i)
	{
		for (int j = 0; j < in_m; ++j)
		{
			const auto glob_i = ii * block_n + i;
			const auto glob_j = jj * block_m + j;

			ifs.read(reinterpret_cast<char*>(
				&data[glob_i *  glob_m + glob_j]), sizeof(double));
		}
	}
}
...
int main(int argc, char** argv)
{
	if (argc != 5)
	{
		std::cerr << "Error: usage <nb, mb, block_n, block_m>\n";
		return 0;
	}
	const auto nb = std::atoi(argv[1]);
	const auto mb = std::atoi(argv[2]);
	const auto block_n = std::atoi(argv[3]);
	const auto block_m = std::atoi(argv[4]);

	const auto glob_n = nb * block_n;
	const auto glob_m = mb * block_m;

	std::vector<double> data(glob_n * glob_m);
	for (int i = 0; i < nb; ++i)
	{
		for (int j = 0; j < mb; ++j)
		{
			read_file(i, j, block_n, block_m, glob_m, data.data());
		}
	}

	save_png(data.data(), glob_n, glob_m, "out.png", 'c');
